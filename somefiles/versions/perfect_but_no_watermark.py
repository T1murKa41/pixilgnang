...
async def accept_meme(call: types.CallbackQuery):
    ...
    for index, item in enumerate(meme_data['album']):
        caption = description if index == 0 else ''
        if item['type'] == 'photo':
            media_group.append(types.InputMediaPhoto(media=item['media'], caption=caption, parse_mode='markdown'))
        elif item['type'] == 'video':
    ...
    # Отправляем уведомление пользователю об отправке мема в канал
